...
function readAgentPort(extPath) {
  try {
    ...
    console.log("[OPSCURE] Reading agent port from:", p);

    const port = fs.readFileSync(p, "utf8").trim();
    console.log("[OPSCURE] agent.port content:", port);

    return port;
  } catch (e) {
    console.error("[OPSCURE] Failed to read agent.port:", e.message);
    return null;
  }
}
    ...
    const timer = setInterval(() => {
      try {
        if (fs.existsSync(portFile)) {
          ...
          if (/^\d+$/.test(port)) {
            clearInterval(timer);
            console.log("[OPSCURE] Fresh agent port detected:", port);
            return resolve(port);
          }
        }
      } catch {}
...
function activate(context) {
  /* =========================================================
     START GO AGENT AS SIDECAR (RACE-CONDITION FIXED)
     ========================================================= */
  try {
    ...
    // 2. Wait until agent writes NEW port
    waitForAgentPort(context.extensionPath)
      .then(port => {
        AGENT_BASE = `http://127.0.0.1:${port}`;
        console.log("[OPSCURE] AGENT_BASE locked to:", AGENT_BASE);
        vscode.window.showInformationMessage(
          "OPSCURE agent connected on port " + port
        );
      })
      .catch(err => {
